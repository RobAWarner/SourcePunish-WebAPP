...
if(isset($_GET['id']) && $_GET['id'] != '' && IsNum($_GET['id']) && $_GET['id'] > 0) {
    ...
    foreach($Servers as $ID=>$Server) {
        ...
        $ServerTable['rows'][] = array('custom'=>'data-sid="'.$ID.'"', 'cols'=>array(array('content'=>'<span class="s-info-vac"><img alt="VAC" title="Valve Anti-Cheat Secure" src="'.HTML_IMAGES.'vac.png" /></span>'), array('content'=>'<span class="s-info-mod"><img alt="'.$Server['mod']['short'].'" title="'.$Server['mod']['name'].'" src="'.HTML_IMAGES_GAMES.$Server['mod']['image'].'" /></span>'), array('content'=>'<span class="s-info-loc">'.($GeoIP!==false?'<img alt="'.$GeoIP['country_code'].'" title="'.$GeoIP['country'].'" src="'.$GeoIP['country_flag'].'" />':'').'</span>'), array('content'=>'<a href="steam://connect/'.$Server['host'].'" title="'.sprintf($GLOBALS['trans'][1208], $Server['host']).'"><span class="s-info-name">'.htmlspecialchars($Server['name']).'</span></a>'), array('content'=>'<span class="s-info-players">-</span>'), array('content'=>'<span class="s-info-map">-</span>')));
    }
    unset($Servers);
