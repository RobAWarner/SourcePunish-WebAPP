...
class Translations {
    ...
    public function __construct($TranslationFolder, $UserLanguage = 'en', $LoadDefaultEnglish = true) {
        ...
        if(!$this->_TranslationFileExists(null, $UserLanguage)) {
            if(!$LoadDefaultEnglish)
                throw new SiteError('translation.missing', 'The folder for the specified user language "'.$UserLanguage.'" is missing', 'Translations', 'class.translations.php');
            $UserLanguage = null;
        }
    ...
    *****************************************/
    ...
    public function _Load($TranslationModule, $Required = false) {
        $DefaultLoaded = false;
        $UserLoaded = false;
        ...
        if(!$Success && $Required)
            throw new SiteError('translation.module.missing', 'Failed to load translation module: "'.$TranslationModule.'" for language: "'.(($this->LoadDefault && !$DefaultLoaded)?'en':$this->Language).'"');
        else
            return $Success;
    }
